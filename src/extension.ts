...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('git-buddy.createNewRepoAction', async (payload: { name: string, branch: string, readme: boolean }) => {
        const folders = vscode.workspace.workspaceFolders;
        if (!folders) { vscode.window.showErrorMessage('Open a workspace folder directory root first.'); return; }
        const rootPath = folders[0].uri.fsPath;
        try {
            if (payload.readme) { fs.writeFileSync(path.join(rootPath, 'README.md'), `# ${payload.name}\n`); }
        ...
        } catch (err: any) { vscode.window.showErrorMessage(`Initialization Mismatch: ${err.message || err}`); }
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('git-buddy.cloneRepoAction', async (repoUrl: string) => {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Cloning remote directory track...`, cancellable: false }, async () => {
            ...
            } catch (err: any) { vscode.window.showErrorMessage(`Clone operation anomaly: ${err.message || err}`); }
        });
    }));
    ...
    let pushCommand = vscode.commands.registerCommand('git-buddy.oneClickPush', async () => {
        ...
        const folders = vscode.workspace.workspaceFolders;
        if (!folders) { vscode.window.showErrorMessage('Open a workspace project folder tracking context.'); return; }
        const rootPath = folders[0].uri.fsPath;
        ...
        const updateStateMapTick = async (idx: number, state: 'completed' | 'active' | 'waiting', desc?: string) => {
        ...
        try {
            ...
            try { await execGitFast(['remote', 'add', 'origin', targetUpstreamUrl], rootPath); } catch { await execGitFast(['remote', 'set-url', 'origin', targetUpstreamUrl], rootPath); }
